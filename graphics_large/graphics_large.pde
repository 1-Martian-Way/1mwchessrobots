...
void setup() {
  ...
  fontA = loadFont("Arial-BoldMT-36.vlw");
  ...
  for (int i = 0; i < cols; i ++ ) {
    for (int j = 0; j < rows; j ++ ) {
      ...
      sprite[i][j] = new Piece((i*large_p + small_p),(j*large_p + small_p),50,50,1,ID);
      // Color 3 = Blue //
      }
      ...
      sprite[i][j] = new Piece((i*large_p + small_p),(j*large_p + small_p),50,50,2,ID);
      // Color 2 = Green //
      }
      }
...
void draw() {
  ...
  //////////////// Origin and Destination ///////////////
      ...
      fill(red_color);
      text("ORIGIN ?", 280, prnt_val);
      }
      
      if (s_enable == 1)
      {
      fill(red_color);
      text("ORIGIN = ", 270, prnt_val);
      text(Origin, 360, prnt_val);
  ...
  fill(255);
  rect(0,prnt_val,200,50);
   
  stroke(0);
  fill(red_color);
  
  rect(0,prnt_val,b,50);
