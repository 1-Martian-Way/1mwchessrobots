...
int h_pos[16] = {96, 97, 98, 99, 100, 101, 102, 103,
               112, 113, 114, 115, 116, 117, 118, 119};

int c_pos[16] = {16, 17, 18, 19, 20, 21, 22, 23,
                 0, 1, 2,  // R2, N2, B2
                 3, 4,     // Q, K
                 5, 6, 7}; // B1, N1, R1
...
/******************* Core Communication *************************/
    ...
    /* If Destination Reached '-' Received from Piece, Send K and L to Comp */
    if (inByte =='-') 
    {
    Serial.print("-"); 
    line_time = 0; 
    ...
    /* No Responses from King back to Piece for following */
    // '{' - King just sends this once to piece to ask it to quit
    //       after it has sent map to the piece
    ...
    if (inByte == '|')
    {
     Serial.print("|"); //May have to add another wait_to_c
     k = 8;
    ...
    /////// 1. Reconfirm KL_comp i.e. Confirm the k and l data 
    /////// coming from comp which was sent to it ////////////
    ...
    {
    L_recvd = num_to_pos();
    Serial.println("L_recvd = " + L_recvd);
    reconfirm_l_comp();
    }
    }
...
{  
   ...
   if (c_pos[i]== K)
   {
    ...
    if (c_ID[i] == 'j') {knight = 1;} 
     /*if (knight_adj == 1) boot_left();*/
    
    if (c_ID[i] == 'o') {knight = 1;}
...
if (send_ctr <0)  // TODO: Change back to < 16
{  
...
void attach_timer(char s_sym)
{
  ...
  if (s_sym == '_') a_timer_sym = '_'; // King -> Piece (Robot Moves e.g. _c12 where 1 is Direction i.e. F,B,L,R,etc and 2 is Step Size)
  if (s_sym == '(') a_timer_sym = '('; // Quit Moves
  if (s_sym == ')') a_timer_sym = ')'; // Quit Moves
  if (s_sym == ';') a_timer_sym = ';'; // Notify Comp
  if (s_sym == '@') a_timer_sym = '@'; // King Send Go
  if (s_sym == ',') a_timer_sym = ','; // Tell piece that human outbounded 
  if (s_sym == '!') a_timer_sym = '!';
  if (s_sym == ':') a_timer_sym = ':';
  if (s_sym == '~') a_timer_sym = '~'; // King => Pieces (Return to position)
  if (s_sym == 'k') a_timer_sym = 'k'; // King -> Comp (Send Robot Move Origin)
  if (s_sym == 'l') a_timer_sym = 'l'; // King -> Comp (Send Robot Move Dest)
  if (s_sym == 'x') a_timer_sym = 'x'; // 
  if (s_sym == '[') a_timer_sym = '['; // King -> Blocking Piece (Direction Data Exchange)
  if (s_sym == ']') a_timer_sym = ']'; // King -> Blocking Piece (Signal to Move to Directed Position)
  if (s_sym == '=') a_timer_sym = '='; // King -> Knight (Go Knight you are free)
  if (s_sym == '{') a_timer_sym = '{'; // King -> Defeated Robot Piece (Quit piece)
...
/////////////// King -> Comp Using Serial.write ///////////
///////////////// SEND k_comp /////////////////////////////
 
 void send_k_comp() 
 {
  comp_flag = 1; 
  Serial.write('^'); Serial.write('k');
  pos_to_num(K);
 }
